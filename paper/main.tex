...
We next ask whether the truthfulness guarantee costs performance. We compare GTMD-RL against round-robin (RR), max-CQI (best-rate), proportional-fair (PF), and the single-lever variant with floors forced always active ($\rhobind\!=\!1$), all driven on identical arrival and channel realizations. Table~\ref{tab:compare} reports the slice-KPI panel at moderate ($\load{=}1.0$) and overloaded ($\load{=}1.3$) operating points; Fig.~\ref{fig:perslice} breaks the overloaded point out per slice.
...
Round-robin        & 16.73 & 2.66 & \textbf{0.019} & 1.90 & 0.99 \\
...
\textbf{GTMD-RL (ours)} & 16.73 & \textbf{2.57} & 0.020 & 1.90 & \textbf{1.00} \\
\midrule
\multicolumn{6}{@{}l}{\textit{Load $\load=1.3$ (overloaded)}}\\
Round-robin        & 23.17 & \textbf{75.4}  & 0.252 & \textbf{3.74} & 0.94 \\
Max-CQI            & 23.17 & 500.0 & \textbf{0.234} & 4.65 & 0.91 \\
Proportional-fair  & 23.17 & 500.0 & 0.276 & 4.04 & 0.86 \\
Floors-always ($\rhobind{=}1$) & 23.17 & 121.9 & 0.340 & 6.08 & \textbf{1.00} \\
\textbf{GTMD-RL (ours)} & 23.17 & 219$^\dagger$ & 0.238 & 4.17 & \textbf{1.00} \\
\bottomrule
\end{tabular}
\vspace{2pt}
{\footnotesize $^\dagger$aggregate p95 is dominated by best-effort mMTC under overload (Fig.~\ref{fig:perslice}); GTMD-RL's SLA-violation rate ties the best non-truthful scheduler to within $0.004$ while it -- uniquely among efficient policies -- also guarantees floors ($1.00$) and is DSIC.}
\end{table}

\begin{figure}[t]
\centering
\includegraphics[width=0.99\columnwidth]{scheduler_perslice.png}
\caption{Per-slice view at overload ($\load{=}1.3$). The aggregate p95 of Table~\ref{tab:compare} is dominated by the delay-tolerant mMTC slice and hides what matters: GTMD-RL keeps the URLLC tail among the best (a), is competitive on priority-weighted SLA (b), and---uniquely among demand-aware policies---guarantees hard floors (c).}
\label{fig:perslice}
\end{figure}

Three findings stand out. First, \emph{truthfulness is essentially free}: at every load GTMD-RL delivers the same aggregate throughput as the throughput-greedy max-CQI policy (arrival-limited below overload), matches PF's fairness, and its SLA-violation rate is within $0.004$ of the best non-truthful scheduler at both operating points, so the incentive guarantee costs no measurable efficiency. Second, \emph{it uniquely pairs that efficiency with hard guarantees}: GTMD-RL keeps the URLLC tail latency ($4.17$\,ms at overload) among the best and far below the $500$\,ms of max-CQI and PF, which starve the cell-edge/low-rate slice, while being the only demand-aware policy with $100\%$ floor satisfaction (the raw baselines drop to $0.86$--$0.99$); Fig.~\ref{fig:perslice} makes the per-slice picture explicit. Third, and directly confirming the theory, the \emph{floors-always-active ($\rhobind\!=\!1$) baseline is measurably worse}: forcing floors on every slot raises the SLA-violation rate to $0.340$ at $\load{=}1.3$, worse than GTMD-RL and even worse than plain RR, because it wastes reserved PRBs on non-binding slots. This is the single-lever barrier of Theorem~\ref{thm:frontier} showing up as a performance penalty: gating floor enforcement by the exogenous $\rhobind$, rather than pinning it to $1$, is what lets the load-gated allocator sit strictly inside the barrier while remaining truthful.
