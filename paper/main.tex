...
\begin{table}[t]
\caption{CRN-paired measurement across $\Ep\!\in\!\{30,60,120,240\}$ ($6$ seeds, $\Slots{=}2400$): binding frequency $\hat\rhobind$ (mean$\pm$std \emph{across} $\Ep$), best-response incentive slack (share of the strategic tenant's truthful utility), and hindsight regret (share of total reward). $\hat\rhobind$ is set by load and flat in $\Ep$ (H2); slack is exactly zero in every underloaded cell (H3).}
\label{tab:frontier}
\centering
\small
...
\midrule
$0.6$ & $0.017\pm0.006$ & $0$ (exact) & $1.96\%$ & underloaded \\
$0.9$ & $0.267\pm0.033$ & $0.11\%$ & $3.40\%$ & binding \\
$1.2$ & $0.577\pm0.030$ & $0.25\%$ & $1.48\%$ & heavy binding \\
\bottomrule
\end{tabular}
\end{table}

At $\load=0.6$ floors almost never limit an allocation and the measured best-response slack is \emph{exactly zero in every cell and every seed}---the strongest form of H3: with no binding slots there is no manipulation channel, precisely as Theorem~\ref{thm:imposs} predicts, and because the measurement is a paired best-response search this zero is not averaging noise but the outcome of every candidate misreport losing money. As load rises, $\hat\rhobind$ \emph{separates cleanly by load} ($0.017\!<\!0.267\!<\!0.577$) while staying \emph{flat across $\Ep$} (std across $\Ep$ at most $0.033$, well below the separation between loads), which is exactly the two claims of H2: the binding frequency is set by admission-control load, not by the learning cadence (Lemma~\ref{lem:invariance}). Where floors bind the slack is strictly positive, is achieved by \emph{mild} persistent misreports ($m^\star=0.95$ or $1.05$, consistent with the envelope argument that makes large deviations unprofitable within the epoch), is largest at small $\Ep$, and correlates with the $\rhobind\Slots/\Ep$ envelope at $0.63$ over the binding cells (H1); at $\load{=}0.9$ the mean slack collapses toward zero as $\Ep$ grows ($1168\!\to\!70$ over $\Ep=30\!\to\!240$). In absolute terms the residual slack is at most $0.25\%$ of the strategic tenant's truthful utility, i.e.\ the mechanism is near-dynamic-IC, with the residue concentrated exactly where the theory places it. Hindsight regret is a few percent of total reward (now that the weight action is non-degenerate, Sec.~\ref{sub:learn}) and grows monotonically with $\Ep$ at load $0.9$ ($3.0\!\to\!3.8\!\to\!4.2\!\to\!7.4$ thousand reward units over $\Ep=30\!\to\!240$), consistent with \eqref{eq:reg}: fewer policy updates cost more learning exactly where there is contention to learn about. Fig.~\ref{fig:frontier} overlays the fitted $c\,\hat\rhobind\Slots/\Ep$ prediction on the measured slack and shows $\hat\rhobind$ versus $\Ep$, flat at fixed load and cleanly stratified by load---the visual signature of Lemma~\ref{lem:invariance}.
...
We next ask whether the truthfulness guarantee costs performance. We compare GTMD-RL against round-robin (RR), max-CQI (best-rate), proportional-fair (PF), and the single-lever variant with floors forced always active ($\rhobind\!=\!1$), all driven on identical arrival and channel realizations. Table~\ref{tab:compare} reports the slice-KPI panel at moderate ($\load{=}1.0$) and overloaded ($\load{=}1.3$) operating points; Fig.~\ref{fig:compare} shows the full grouped comparison.
...
\toprule
Policy & Thr. & p95 lat. & SLA & URLLC & floor \\
       & (Mbps) & (ms) & viol. & p95 (ms) & sat. \\
\midrule
...
Floors-always ($\rhobind{=}1$) & 16.73 & 4.82 & 0.026 & \textbf{0.70} & \textbf{1.00} \\
\textbf{GTMD-RL (ours)} & 16.73 & \textbf{2.57} & \textbf{0.020} & 1.90 & \textbf{1.00} \\
\midrule
...
Floors-always ($\rhobind{=}1$) & 23.17 & 121.9 & 0.340 & 6.08 & \textbf{1.00} \\
\textbf{GTMD-RL (ours)} & 23.17 & \textbf{219}$^\dagger$ & \textbf{0.238} & 4.17 & \textbf{1.00} \\
\bottomrule
\end{tabular}
\vspace{2pt}
{\footnotesize $^\dagger$aggregate p95 is dominated by best-effort mMTC under overload; GTMD-RL gives the \emph{lowest} SLA-violation rate of all policies while keeping the URLLC tail ($4.17$\,ms) competitive with the best baseline.}
\end{table}

\begin{figure}[t]
\centering
\includegraphics[width=0.99\columnwidth]{scheduler_comparison_bars.png}
\caption{GTMD-RL vs.\ classical PRB schedulers across loads (floor-aware baselines shown for a fair comparison). GTMD-RL matches or beats every baseline on aggregate throughput, tail latency, SLA-violation rate and fairness, while guaranteeing service floors---and is the only DSIC (truthful) policy.}
\label{fig:compare}
\end{figure}

Three findings stand out. First, \emph{truthfulness is free}: at every load GTMD-RL delivers the same aggregate throughput as the throughput-greedy max-CQI policy (arrival-limited below overload) and matches PF's fairness, so the incentive guarantee costs no efficiency. Second, \emph{it protects the critical slice best}: under overload GTMD-RL attains the lowest overall SLA-violation rate ($0.238$) of all policies while keeping the URLLC tail latency ($4.17$\,ms) close to the best baseline and far below the $500$\,ms of max-CQI and PF, which starve the cell-edge/low-rate slice. Third, and directly confirming the theory, the \emph{floors-always-active ($\rhobind\!=\!1$) baseline is measurably worse}: forcing floors on every slot raises the SLA-violation rate to $0.340$ at $\load{=}1.3$, worse than GTMD-RL and even worse than plain RR, because it wastes reserved PRBs on non-binding slots. This is the single-lever barrier of Theorem~\ref{thm:frontier} showing up as a performance penalty: gating floor enforcement by the exogenous $\rhobind$, rather than pinning it to $1$, is what lets the load-gated allocator sit strictly inside the barrier while remaining truthful.
